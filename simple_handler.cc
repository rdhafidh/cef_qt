...
GetViewRect(CefRefPtr<CefBrowser> browser, CefRect& rect)
{
 ...
 // rect.x= m_windowRect.x();
  //rect.y = m_windowRect.y();
  rect.width = m_windowRect.width();
  rect.height = m_windowRect.height();
}
...
void SimpleHandler::OnPaint(CefRefPtr<CefBrowser> browser,
                       ...
                       int height)
{
	HBITMAP hBitmap = CreateBitmap(m_windowRect.width(),m_windowRect.height(),1,32,buffer);
